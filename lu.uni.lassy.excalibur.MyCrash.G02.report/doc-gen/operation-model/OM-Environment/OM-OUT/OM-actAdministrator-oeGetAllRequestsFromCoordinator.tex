...
The \msrcode{oeGetAllRequestsFromCoordinator} operation has the following properties:
	...
	\adddoublerow{oeGetAllRequestsFromCoordinator}{sent to get all the requests checked by the actor ActCoordinator in the system’s post state.}
	...
	\addnumberedsinglerow{PostF}{the actor ActAdministrator gets a list of requests to handle if there are pending requests.
	the administrator actor is also informed about the non-satisfaction of its request.}
