...
The \msrcode{oeLogout} operation has the following properties:
	...
	\addnumberedsinglerow{PreP}{PreP 2 the actor is currently logged in ! (i.e. the associated ctAuthenticated instance is considered
	logged)}
	...
	\addnumberedsinglerow{PostF}{PostF 1 a logout confirmation message is sent to the actor (n.b. the logged status is changed as a
	post-protocol condition) }

	\addrowheading{Post-Condition (protocol)}
	\addnumberedsinglerow{PostP}{PostP 1 the actor is known to be logged out ! (i.e. the associated ctAuthenticated instance with
	given login and password is considered logged out) }
	\end{operationmodel}
